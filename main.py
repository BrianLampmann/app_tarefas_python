...
#tkinter = biblioteca para criação de janelas onde fornece varios widgets (label, frame, canvas)
#bg = background
#fg = foreground (cor letras)
#label = um widget (ferramenta) para o tkinter para printar na tela 
#frame (quadro) = usado como container dentro do tkinter
#fill: Define se o widget deve expandir-se para preencher o espaço disponível no contêiner. Pode ser tk.NONE (padrão), tk.X (preenche na direção horizontal), tk.Y (preenche na direção vertical), ou tk.BOTH (preenche em ambas as direções).
...
def adicionar_item_tarefa(tarefa):
    ...
    botao_deletar = tk.Button(frame_tarefa, image=icon_deletar, command=lambda f=frame_tarefa: deleter_tarefa(f), bg="white", relief=tk.FLAT)
...
def deleter_tarefa(frame_tarefa):
...
icon_deletar = PhotoImage(file="delete.png").subsample(20,20)
...
# Criando um frame para a lista de tarefas com rolagem (scroll)
